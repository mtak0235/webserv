...
std::string Server::_getBody(std::string file, int k)
{
	...
	if (!_requestMethod.compare("GET"))
	{
		...
		{
			body = _cgi.getCgiResponse(this->_request, _nowLocation.getCgiPath(), rootPulsFile); // 이거랑
	...
	else if (!_requestMethod.compare("POST"))
	{
		...
		{
			body = _cgi.getCgiResponse(this->_request, _nowLocation.getCgiPath(), rootPulsFile); //같음
			_statusCode = _cgi.getStatusCode();
		}
		else
			_statusCode = 403;
